...
client.on('ready', async () => {
 ...
 client.user.setActivity(`V E N U S | G U A R D`, { type:'WATCHING' })
  
  console.log("GUARD AKTİF BABBA")
});
...
let kanal = await db.fetch(`antiraidK_${member.guild.id}`)== "anti-raid-aç"
  ...
  if (member.user.bot === true) {
    ...
    let darknesguardv2 = new Discord.MessageEmbed()
      ...
      .setDescription(`**${member.user.tag}** (${member.id}) adlı bota bir yetkili izin verdi eğer kaldırmak istiyorsanız **v!bot-izni kaldır <botid>**.`);
    darknesyt.send(darknesguardv2);
     } else {
       let izinverilmemişbot = new Discord.MessageEmbed()
      .setColor("RANDOM")
      .setThumbnail(member.user.avatarURL())
      .setDescription("**" + member.user.tag +"**" + " (" + member.id+ ") " + "adlı bot sunucuya eklendi ve banladım eğer izin vermek istiyorsanız **" + "v!bot-izni ver <botid>**")
...
client.on("guildMemberAdd", member => {
  ...
  moment.locale("tr")
   ...
   var rol = member.guild.roles.cache.get("823185163273764886") //Cezalı Rol İD
   var kayıtsız = member.guild.roles.cache.get("823185211356348446") //Alınacak Rol İD
